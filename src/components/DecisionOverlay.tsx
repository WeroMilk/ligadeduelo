    ...
    <div className="absolute inset-0 z-[70] flex items-center justify-center bg-black/55 p-2">
      <div className="w-full max-w-sm overflow-hidden rounded-2xl border-2 border-[#C9A84C] bg-[#141B2D] p-3 shadow-[0_0_40px_rgba(201,168,76,0.25)] space-y-1.5">
        <div className="flex items-center justify-between gap-2">
          ...
          {showTimer ? (
            <span className="text-[10px] font-mono text-[#8B9BB4]">{secondsLeft}s</span>
          ) : (
            <span className="text-[9px] text-[#8B9BB4]">Sin límite</span>
          )}
        </div>
        {teamColor && (
          <p className={`text-[10px] font-bold ${teamColor === 'blue' ? 'text-[#3498DB]' : 'text-[#E74C3C]'}`}>
              ...
              {assistOptions.map(c => {
                ...
                return (
                  <button
                    ...
                    className="min-h-9 rounded-lg border border-[#2A3550] bg-[#0A0E1A] px-2 py-1 flex items-center gap-2 text-left"
                  >
                    {def.image ? (
                      <img
                        src={def.image}
                        alt={def.name}
                        className="h-7 w-7 shrink-0 rounded-full object-cover border border-[#C9A84C]/50"
                      />
                    ) : (
                      <div
                        className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[#C9A84C]/50 text-[9px] font-bold text-white"
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] font-bold leading-tight text-[#F0E6D2] whitespace-normal break-words">
                        {def.name}
                      </p>
                      <p className="text-[8px] text-[#8B9BB4] leading-none">{ROLE_NAMES[def.role]}</p>
                    </div>
                  </button>
                );
              })}
