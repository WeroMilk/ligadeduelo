...
export default function TournamentWin() {
  ...
  const ring = 'border-[#C9A84C] shadow-[0_0_16px_rgba(201,168,76,0.45)]';
  const skinTint = 'hue-rotate-[8deg] saturate-125 brightness-110';

  if (!playerWon) {
    return (
      <div className="screen-center relative bg-[#0A0E1A] px-4 py-3 md:py-6 safe-top safe-chrome-x safe-bottom">
        <div className="relative z-10 flex w-full max-w-md flex-col items-center gap-3 md:gap-4 py-2">
    ...
    <div className="screen-center relative bg-[#0A0E1A] px-4 py-3 md:py-6 safe-top safe-chrome-x safe-bottom">
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col items-center gap-3 md:gap-4 py-2">
        ...
        <div className="text-center shrink-0">
          ...
          <h1
            className="text-xl sm:text-3xl font-bold text-[#F1C40F]"
        ...
        {titles.length > 0 && (
          <div className="w-full flex flex-wrap justify-center gap-2 shrink-0">
            {titles.map(t => (
              <span
                key={t}
                className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border border-[#C9A84C]/50 bg-[#C9A84C]/15 text-[#C9A84C]"
              >
        ...
        <div className="w-full rounded-xl border p-2.5 md:p-3 shrink-0 bg-[#141B2D] border-[#C9A84C]/30">
          <p className="text-xs uppercase tracking-wider text-center mb-2 text-[#C9A84C]">
            Marco Dorado · Tu equipo
          </p>
            ...
            {champions.map(c => {
              ...
              return (
                <div key={c.instanceId} className="flex flex-col items-center gap-1 w-14">
                  ...
                  <span className="text-[9px] text-[#8B9BB4] truncate w-full text-center">{def.name}</span>
                </div>
              );
            })}
          </div>
